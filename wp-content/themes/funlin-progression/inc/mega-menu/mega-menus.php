...
if( ! class_exists( 'ProgressionFrontendWalker' ) ) {
	class ProgressionFrontendWalker extends Walker_Nav_Menu {
		...
		private $menu_megamenu_visbility = '';
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$this->menu_megamenu_visbility = get_post_meta( $item->ID, '_menu_item_progression_studios_megamenu_visibility', true);
			...
			if( $depth === 1 && $this->menu_megamenu_status == "enabled" ) {
				...
				if( $this->menu_megamenu_title != 'disabled' && $title != "-" && $title != '"-"' ) {
					...
					if( ! empty( $this->menu_megamenu_icon ) ) {
						$title_enhance = '<span class="progression-megamenu-icon"><i class="' . $this->menu_megamenu_icon. '"></i></span>';
					}
				...
				if( ! empty( $this->menu_megamenu_icon )) {
					$item_output .= '<a ' . $attributes . '><span class="progression-studios-menu-title"><span class="progression-megamenu-icon text-menu-icon"><i class="' . $this->menu_megamenu_icon . '"></i></span>';
				} else {
    ...
    class ProCoreMegaMenus extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( esc_html__( '%s (Invalid)', 'funlin-progression' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( esc_html__('%s (Pending)', 'funlin-progression'), $item->title );
			}
			...
			<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo esc_html( implode(' ', $classes ) ); ?>">
				<dl class="menu-item-bar">
					<dt class="menu-item-handle">
						<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo esc_html( $submenu_text); ?>><?php esc_html_e( 'sub item', 'funlin-progression' ); ?></span></span>
						<span class="item-controls">
							...
							<span class="item-order hide-if-js">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'funlin-progression'); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										add_query_arg(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'funlin-progression'); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'funlin-progression'); ?>" href="<?php
								echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
							?>"><?php esc_html_e( 'Edit Menu Item', 'funlin-progression' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
					<?php if( 'custom' == $item->type ) : ?>
						<p class="field-url description description-wide">
							<label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'URL', 'funlin-progression' ); ?><br />
					...
					<p class="description description-thin">
						<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Navigation Label', 'funlin-progression' ); ?><br />
							<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
						</label>
					</p>
					<p class="description description-thin">
						<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Title Attribute', 'funlin-progression' ); ?><br />
					...
					<p class="field-link-target description">
						<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Open link in a new window/tab', 'funlin-progression' ); ?>
						</label>
					</p>
					<p class="field-css-classes description description-thin">
						<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'CSS Classes (optional)', 'funlin-progression' ); ?><br />
							<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
						</label>
					</p>
					<p class="field-xfn description description-thin">
						<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Link Relationship (XFN)', 'funlin-progression' ); ?><br />
							<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
						</label>
					</p>
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Description', 'funlin-progression' ); ?><br />
							<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
							<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'funlin-progression'); ?></span>
						</label>
					</p>
					<div class="clear"></div>
					<div class="progression-mega-menu-options">
						...
						<!--p class="field-megamenu-notif description description-wide">
							<label for="edit-menu-item-megamenu-notif-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Menu Mini Banner Text', 'funlin-progression' ); ?>
						...
						<p class="field-megamenu-status description description-wide">
							<label for="edit-menu-item-megamenu-status-<?php echo esc_attr($item_id); ?>">
								...
								<strong><?php esc_html_e( 'Enable Mega Menu', 'funlin-progression' ); ?></strong>
							</label>
						</p>
						...
						<p class="field-megamenu-columns description description-wide">
							<label for="edit-menu-item-megamenu-columns-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Mega Menu Number of Columns', 'funlin-progression' ); ?>
								<select id="edit-menu-item-megamenu-columns-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-columns" name="menu-item-progression-megamenu-columns[<?php echo esc_attr($item_id); ?>]">
									<option value="1" <?php selected( $item->progression_studios_megamenu_columns, '1' ); ?>><?php esc_html_e( '1 Column', 'funlin-progression' ); ?></option>
									<option value="2" <?php selected( $item->progression_studios_megamenu_columns, '2' ); ?>><?php esc_html_e( '2 Columns', 'funlin-progression' ); ?></option>
									<option value="3" <?php selected( $item->progression_studios_megamenu_columns, '3' ); ?>><?php esc_html_e( '3 Columns', 'funlin-progression' ); ?></option>
									<option value="4" <?php selected( $item->progression_studios_megamenu_columns, '4' ); ?>><?php esc_html_e( '4 Columns', 'funlin-progression' ); ?></option>
									<option value="5" <?php selected( $item->progression_studios_megamenu_columns, '5' ); ?>><?php esc_html_e( '5 Columns', 'funlin-progression' ); ?></option>
									<option value="6" <?php selected( $item->progression_studios_megamenu_columns, '6' ); ?>><?php esc_html_e( '6 Columns', 'funlin-progression' ); ?></option>
								</select>
							</label>
						</p>
						<p class="field-megamenu-title description description-wide">
							<label for="edit-menu-item-megamenu-title-<?php echo esc_attr($item_id); ?>">
								...
								<?php esc_html_e( 'Disable Mega Menu Column Title', 'funlin-progression' ); ?>
							</label>
						</p>
						...
						<p class="field-megamenu-visibility description description-wide">
							<label for="edit-menu-item-megamenu-icon-<?php echo esc_attr($item_id); ?>">
								<?php wp_kses( esc_html_e( 'Visibility logic', 'funlin-progression' ) , true); ?><?php echo' (<a href="' ?>https://codex.wordpress.org/Conditional_Tags" target="_blank">?</a>)
								<input type="text" id="edit-menu-item-megamenu-visibility-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-visibility" name="menu-item-progression-megamenu-visibility[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr($item->progression_studios_megamenu_visibility); ?>" />
							</label>
						</p>
						...
						<p class="field-megamenu-icon description description-wide">
							<label for="edit-menu-item-megamenu-icon-<?php echo esc_attr($item_id); ?>">
								<?php wp_kses( esc_html_e( 'Menu Icon', 'funlin-progression' ) , true); ?><?php echo' (<a href="' ?>https://fontawesome.com/icons?d=gallery&m=free" target="_blank">Font-Awesome Icon List</a>)
						...
						<p class="field-megamenu-widgetarea description description-wide">
							<label for="edit-menu-item-megamenu-widgetarea-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Mega Menu Widget Area', 'funlin-progression' ); ?>
								<select id="edit-menu-item-megamenu-widgetarea-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-widgetarea" name="menu-item-progression-megamenu-widgetarea[<?php echo esc_attr($item_id); ?>]">
									<option value="0"><?php esc_html_e( 'Select Widget Area', 'funlin-progression' ); ?></option>
									<?php
					...
					<p class="field-move hide-if-no-js description description-wide">
						<label>
							<span><?php esc_html_e( 'Move', 'funlin-progression' ); ?></span>
							<a href="#" class="menus-move-up"><?php esc_html_e( 'Up one', 'funlin-progression' ); ?></a>
							<a href="#" class="menus-move-down"><?php esc_html_e( 'Down one', 'funlin-progression' ); ?></a>
							<a href="#" class="menus-move-left"></a>
							<a href="#" class="menus-move-right"></a>
							<a href="#" class="menus-move-top"><?php esc_html_e( 'To the top', 'funlin-progression' ); ?></a>
						</label>
					</p>
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
							<p class="link-to-original">
								...
								esc_html__('Original: %s', 'funlin-progression'), 
						...
						); ?>"><?php esc_html_e( 'Remove', 'funlin-progression' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
							?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'funlin-progression'); ?></a>
					</div>
					<div class="clear"></div>
    ...
    class ProgressionStudiosMegaMenu extends ProgressionStudiosMegaMenuFramework {
		...
		function visibility_check( $items, $menu, $args ) {
			...
			foreach ( $items as $key => $item ) {
				...
				if ( $logic = get_post_meta( $item->ID, '_menu_item_progression_studios_megamenu_visibility', true ) ) {
					eval( '$visible = ' . $logic . ';' );
				} else {
					$visible = true;
				}
		...
		function remove_visibility_meta( $post_id ) {
			if ( is_nav_menu_item( $post_id ) ) {
				delete_post_meta( $post_id, '_menu_item_progression_studios_megamenu_visibility' );
			}
		}
        ...
        function save_custom_fields( $menu_id, $menu_item_db_id, $args ) {
			...
			$field_name_suffix = array( 'status', 'width', 'columns', 'title', 'widgetarea', 'icon', 'notif' , 'visibility');
        ...
        function add_data_to_menu( $menu_item ) {
	            ...
	            $menu_item->progression_studios_megamenu_visibility = get_post_meta( $menu_item->ID, '_menu_item_progression_studios_megamenu_visibility', true );
